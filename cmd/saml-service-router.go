...
func registerSAMLRouter(mux *router.Router) error {
	...
	samlRouter.Methods("POST").Path("/SAML2/ACS").HandlerFunc(samlSP.SAMLAssertionConsumerHandler)
	samlRouter.Methods("GET").Path("/login").HandlerFunc(samlSP.SAMLLoginHandler)
	samlRouter.Methods("GET").Path("/logout").HandlerFunc(samlSP.SAMLLogoutHandler)
